...
const bossNames: Record<BossId, string> = { tabelldraken: 'Tabelldraken', brakbjornen: 'Bråkbjörnen', monsterormen: 'Mönsterormen', plottrig: 'Plottrig', stenjatten: 'Stenjätten' }
  ...
  return <main className="character-demo">
    <div className="character-demo__inner">
      ...
      <nav className="character-demo__tabs" aria-label="Prototypvyer">
        <button aria-pressed={view === 'motion'} onClick={() => { setView('motion'); playMotion('play') }}>Bossfight v10</button>
      ...
      <div className="character-demo__grid">
        <section className="character-demo__panel">
          <h2>{view === 'figure' ? 'Garderob' : view === 'battle' ? 'Stridskontroll' : 'Produktionsprov v10'}</h2>
          {view === 'motion' ? <>
      ...
      <footer className="character-demo__footer"><span>V10 testar samma återanvändbara bossystem med fem världsbossar, senast Stenjätten.</span><span>Ingen matte, valuta, sparfil eller vanlig appdata ändras.</span></footer>
    </div>
  </main>
}
