...
function CharacterPreview() {
  ...
  const [view, setView] = useState<'figure' | 'battle' | 'motion'>('figure')
    ...
    <div className="character-demo__inner">
      ...
      <div className="character-demo__grid">
        <section className="character-demo__panel">
          <h2>{view === 'figure' ? 'Garderob' : view === 'battle' ? 'Stridskontroll' : 'Attackkontroll v2'}</h2>
          {view === 'motion' ? <>
        ...
        <section className="character-demo__panel">
          <h2>{view === 'figure' ? 'Posprov' : view === 'battle' ? 'Rätt/fel-reaktion' : 'Filmisk bågattack'}</h2>
          {view === 'motion' ? <>
