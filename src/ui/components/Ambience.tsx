...
/* Tunn, mjuk molnslöja — suddig och genomskinlig så den smälter in i de
   målade himlarna i stället för att se ut som en tecknad puffe. */
function Cloud() {
  return (
    <svg viewBox="0 0 240 60" width="100%" height="100%" aria-hidden="true"
      style={{ display: 'block', filter: 'blur(3.5px)' }}>
      <g fill="#FFFFFF">
        <ellipse cx="62" cy="36" rx="58" ry="12" />
        <ellipse cx="112" cy="28" rx="50" ry="15" />
        <ellipse cx="164" cy="35" rx="50" ry="11" />
        <ellipse cx="118" cy="42" rx="92" ry="8" />
      </g>
    </svg>
  )
}
...
type FlyerKind = 'butterfly' | 'bat' | 'fish' | 'leaf' | 'cloud'
interface Flyer {
  kind: FlyerKind
  top: string       // vertikalt läge (%)
  size: number      // px (bredd)
  dur: number       // s för att korsa scenen
  delay: number     // s (negativ = redan synlig vid start)
  dir?: 'ltr' | 'rtl'
  color?: string
  opacity?: number
}
...
/* Moln driver långsamt (60–120 s), tunna och svaga. Fjärilar/fladdermöss/
   fiskar/löv ger liv nära marken. Negativa delays gör att några syns direkt. */
const SCENES: Record<string, Scene> = {
  riket: {
    flyers: [
      { kind: 'cloud', top: '10%', size: 240, dur: 105, delay: -40, opacity: 0.38 },
      { kind: 'cloud', top: '26%', size: 180, dur: 130, delay: -80, dir: 'rtl', opacity: 0.3 },
      { kind: 'cloud', top: '62%', size: 210, dur: 120, delay: -20, opacity: 0.26 },
    ],
    particles: [],
  },
  'talens-dal': {
    flyers: [
      { kind: 'cloud', top: '10%', size: 200, dur: 100, delay: -30, opacity: 0.4 },
      { kind: 'cloud', top: '20%', size: 150, dur: 120, delay: -70, dir: 'rtl', opacity: 0.3 },
      { kind: 'butterfly', top: '56%', size: 30, dur: 24, delay: -10 },
      { kind: 'butterfly', top: '68%', size: 24, dur: 28, delay: 6, dir: 'rtl', color: '#B8E0A0' },
    ],
    particles: [{ kind: 'glint', x: 50, y: 42, glow: '#FFE7A8', n: 4 }],
  },
  multiplikationsskogen: {
    flyers: [
      { kind: 'cloud', top: '9%', size: 190, dur: 110, delay: -50, opacity: 0.32 },
      { kind: 'leaf', top: '0%', size: 20, dur: 13, delay: -4, color: '#C98A3A' },
  ...
  brakberget: {
    flyers: [
      { kind: 'cloud', top: '12%', size: 210, dur: 115, delay: -30, opacity: 0.42 },
      { kind: 'cloud', top: '24%', size: 160, dur: 140, delay: -90, dir: 'rtl', opacity: 0.3 },
    ],
    particles: [{ kind: 'flake', x: 50, y: 10, glow: '#FFFFFF', n: 10 }],
  },
  monsterskogen: {
    flyers: [
      { kind: 'bat', top: '20%', size: 30, dur: 20, delay: -6 },
  ...
  'formernas-berg': {
    flyers: [
      { kind: 'cloud', top: '11%', size: 200, dur: 110, delay: -40, opacity: 0.36 },
      { kind: 'butterfly', top: '58%', size: 26, dur: 26, delay: -12, color: '#8FE8F0' },
    ],
    particles: [{ kind: 'glint', x: 48, y: 52, glow: '#8FE8F0', n: 6 }, { kind: 'glint', x: 76, y: 46, glow: '#B8F0F4', n: 4 }],
  },
  diagramoarna: {
    flyers: [
      { kind: 'cloud', top: '9%', size: 210, dur: 100, delay: -25, opacity: 0.4 },
      { kind: 'cloud', top: '22%', size: 160, dur: 130, delay: -85, dir: 'rtl', opacity: 0.3 },
  ...
  sambandsgrottan: {
    flyers: [
      { kind: 'bat', top: '18%', size: 30, dur: 18, delay: -5 },
  ...
  return (
    <div aria-hidden="true" style={{ position: 'absolute', inset: 0, zIndex: 1, pointerEvents: 'none', overflow: 'hidden' }}>
      {cfg.flyers.map((f, i) => {
        ...
        const h = f.kind === 'cloud' ? f.size * 0.26 : f.kind === 'leaf' ? f.size : f.size * 0.6
        return (
          <span key={i} style={{
            position: 'absolute', top: f.top, left: leftAnchor ?? 0, width: f.size, height: h,
            opacity: f.opacity ?? 1, animation: anim, willChange: 'left, transform',
          }}>
            {/* Inre vaggning så rörelsen inte är helt spikrak (moln/fisk/löv sköter sig själva). */}
            <span style={{
