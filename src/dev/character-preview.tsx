...
const bossNames: Record<BossId, string> = { tabelldraken: 'Tabelldraken', brakbjornen: 'Bråkbjörnen', monsterormen: 'Mönsterormen', plottrig: 'Plottrig' }
  ...
  return <main className="character-demo">
    <div className="character-demo__inner">
      ...
      <nav className="character-demo__tabs" aria-label="Prototypvyer">
        <button aria-pressed={view === 'motion'} onClick={() => { setView('motion'); playMotion('play') }}>Bossfight v9</button>
      ...
      <div className="character-demo__grid">
        <section className="character-demo__panel">
          <h2>{view === 'figure' ? 'Garderob' : view === 'battle' ? 'Stridskontroll' : 'Produktionsprov v9'}</h2>
          {view === 'motion' ? <>
      ...
      <footer className="character-demo__footer"><span>V9 testar samma återanvändbara bossystem med Tabelldraken, Bråkbjörnen, Mönsterormen och Plottrig.</span><span>Ingen matte, valuta, sparfil eller vanlig appdata ändras.</span></footer>
    </div>
  </main>
}
