...
export function PetHomeScreen(){
 ...
 return <main className={`pet-page screen-fade ${moving?'':'pet-still'}`}>
  ...
  :discovery?<section className="pet-discovery" style={{backgroundImage:`url(${art(`world/${species.worldId}`)})`}}>
   ...
   {view==='pets'&&<PetHouse pets={pets} items={items} canInteract={left>0} onRename={(petId,name)=>store.changeCamp({type:'rename',petId,name})} onBed={(petId,point)=>store.changeCamp({type:'bed',petId,point})} onPacking={()=>{open('packing');setTab('owned')}}/>}
